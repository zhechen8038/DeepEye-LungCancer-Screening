        ...
        response = requests.post(
            ...
            json={
                ...
                'messages': [
                    {
                        'role': 'system',
                        'content': '你是一位专业的医生，专注于息肉医疗健康领域。你的回答应尽可能详细、准确，并使用 Markdown 格式（包括标题、列表、代码块等）进行排版，以便用户更好地阅读和理解。'
                    },
...
@app.route('/go_to_indexFB')
def index_fb():
    return render_template('indexFB.html')

@app.route('/go_to_AIpage')
def ai_page():
    return render_template('AIpage.html')

@app.route('/go_to_First')
def fir_page():
...
def register_page():
    return render_template('regist.html')  # 单独的注册页面
