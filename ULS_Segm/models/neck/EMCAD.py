...
#     def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu'):
...
#         self.lgag3 = LGAG(F_g=channels[1], F_l=channels[1], F_int=channels[1]//2, kernel_size=lgag_ks, groups=channels[1]//2, activation=activation)
...
#         self.lgag2 = LGAG(F_g=channels[2], F_l=channels[2], F_int=channels[2]//2, kernel_size=lgag_ks, groups=channels[2]//2, activation=activation)
...
#         self.lgag1 = LGAG(F_g=channels[3], F_l=channels[3], F_int=int(channels[3]/2), kernel_size=lgag_ks, groups=int(channels[3]/2), activation=activation)
...
#         self.stage2_Conv3 = Conv3Relu(120 * 2, 120)   # 降维
#         self.stage3_Conv3 = Conv3Relu(120 * 4, 120)
#         self.stage4_Conv3 = Conv3Relu(120 * 8, 120)
#         self.scn41= AlignedModulev2PoolingAtten(120 , 120, 120)
#         self.scn31= AlignedModulev2PoolingAtten(120 , 120, 120)
#         self.scn21= AlignedModulev2PoolingAtten(120 , 120, 120)
#         self.final_Conv = Conv3Relu(120 * 4, 120)
...
#         d1 = self.sab(d1)*d1
#         d1 = self.mscb1(d1)
#         d4=self.scn41(d1, self.stage4_Conv3(d4))
#         d3=self.scn31(d1, self.stage3_Conv3(d3))
#         d2=self.scn21(d1, self.stage2_Conv3(d2))


#         feature = self.final_Conv(torch.cat([d1, d2, d3, d4], 1))
#         return feature 
...
class EMCAD(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu'):
        ...
        self.sab = SAB()
        # fuse
        # self.scn41= AlignedModulev2PoolingAtten(channels[3] , channels[3], channels[3])
        # self.scn31= AlignedModulev2PoolingAtten(channels[3] , channels[3], channels[3])
        # self.scn21= AlignedModulev2PoolingAtten(channels[3] , channels[3], channels[3])
        # self.stage2_Conv3 = Conv3Relu(channels[3] * 2, channels[3])   # 降维
        # self.stage3_Conv3 = Conv3Relu(channels[3] * 4, channels[3])
        # self.stage4_Conv3 = Conv3Relu(channels[3] * 8, channels[3])

        # self.final_Conv = Conv3Relu(channels[3] * 4, channels[3])
    ...
    def forward(self, x, skips):
        ...
        # [d1, d2, d3, d4] = self.drop([d1, d2, d3, d4])  # dropblock

        # d = self.final_Conv(torch.cat([d1, d2, d3, d4], 1))
