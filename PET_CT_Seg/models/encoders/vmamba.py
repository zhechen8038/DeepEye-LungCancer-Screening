...
from models.encoders.local_vmamba.local_scan import local_scan_bchw,local_reverse,local_scan
...
if True:
    ...
    class CrossMerge_local(torch.autograd.Function):
        ...
        def forward(ctx, ys: torch.Tensor):
            ...
            ys = ys.view(B, K, D, -1)
            local=8
            ys[:, 2] = local_reverse(ys[:, 2],local, H, W, flip=False)
            ys[:, 3] = local_reverse(ys[:, 3], local, H, W, flip=False,column_first=True)
            ys[:, 6] = local_reverse(ys[:, 6], local, H, W, flip=False)
            ys[:, 7] = local_reverse(ys[:, 7], local, H, W, flip=False,column_first=True)
            ys = ys[:, 0:4] + ys[:, 4:].flip(dims=[-1]).view(B, 4, D, -1)
            y = ys[:, 0]+ys[:,2] + ys[:, 1].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, D, -1)+\
        ...
        def backward(ctx, x: torch.Tensor):
            ...
            xs[:, 0] = x
            xs[:, 2] = local_scan_bchw(x.view(B, C, H, W), local, H, W, flip=False)
            xs[:, 1] = x.view(B, C, H, W).transpose(dim0=2, dim1=3).flatten(2, 3)
            xs[:, 3] = local_scan_bchw(x.view(B, C, H, W), local, H, W, flip=False,column_first=True)

            xs[:, 4:] = torch.flip(xs[:, 0:4], dims=[-1])
    ...
    class CrossScan_multimodal_k4(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor):
            # B, C, H, W -> B, 4, C, 2 * H * W
            B, C, H, W = x_rgb.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_rgb.new_empty((B, 4, C, 2 * H * W))
            xs_fuse[:, 0] = torch.concat([x_rgb.flatten(2, 3), x_e.flatten(2, 3)], dim=2)
            xs_fuse[:, 1] = torch.concat([x_rgb.transpose(dim0=2, dim1=3).flatten(2, 3), x_e.transpose(dim0=2, dim1=3).flatten(2, 3)], dim=2)
            xs_fuse[:, 2:4]= torch.flip(xs_fuse[:, 0:2], dims=[-1])
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            B, C, H, W = ctx.shape
            L = 2 * H * W
            ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, -1, L)  # B, 2,d, 2 * H * W
            y1= ys[:, :,:, :H * W]
            y2= ys[:, :,:, H * W:]

            y1 = y1[:, 0] + y1[:, 1].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, -1, L//2)
            y2 = y2[:, 0] + y2[:, 1].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, -1, L//2)


            # get B, d, H,W
            return y1.view(B, -1, H, W), y2.view(B, -1, H, W)
    class CrossScan_multimodal_local(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor):
            # B, C, H, W -> B, 4, C, 2 * H * W
            local = 8
            B, C, H, W = x_rgb.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_rgb.new_empty((B, 8, C, 2 * H * W))
            x_rgb_local = local_scan_bchw(x_rgb, local, H, W, flip=False)
            x_e_local = local_scan_bchw(x_e, local, H, W, flip=False)
            x_rgb_local_h = local_scan_bchw(x_rgb, local, H, W, flip=False, column_first=True)
            x_e_local_h = local_scan_bchw(x_e, local, H, W, flip=False, column_first=True)
            xs_fuse[:, 0] = torch.concat([x_rgb.flatten(2, 3), x_e.flatten(2, 3)], dim=2)
            xs_fuse[:, 2] = torch.concat([x_rgb_local, x_e_local], dim=2)
            xs_fuse[:, 1] = torch.concat([x_rgb.transpose(dim0=2, dim1=3).flatten(2, 3), x_e.transpose(dim0=2, dim1=3).flatten(2, 3)], dim=2)
            xs_fuse[:, 3] = torch.concat([x_rgb_local_h, x_e_local_h], dim=2)

            xs_fuse[:, 4:]= torch.flip(xs_fuse[:, 0:4], dims=[-1])
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            L = 2 * H * W
            local = 8
            ys[:, 2,:,:H*W] = local_reverse(ys[:, 2,:,:H*W], local, H, W, flip=False)
            ys[:, 2, :, H * W:] = local_reverse(ys[:, 2, :, H * W:], local, H, W, flip=False)

            ys[:, 3, :, :H * W] = local_reverse(ys[:, 3, :, :H * W], local, H, W, flip=False,column_first=True)
            ys[:, 3, :, H * W:] = local_reverse(ys[:, 3, :, H * W:], local, H, W, flip=False,column_first=True)

            ys[:, 6, :, :H * W] = local_reverse(ys[:, 6, :, :H * W], local, H, W, flip=False)
            ys[:, 6, :, H * W:] = local_reverse(ys[:, 6, :, H * W:], local, H, W, flip=False)

            ys[:, 7, :, :H * W] = local_reverse(ys[:, 7, :, :H * W], local, H, W, flip=False, column_first=True)
            ys[:, 7, :, H * W:] = local_reverse(ys[:, 7, :, H * W:], local, H, W, flip=False, column_first=True)

            ys = ys[:, 0:4] + ys[:, 4:].flip(dims=[-1]).view(B, 4, C, L)
            y1= ys[:, :,:, :H * W]
            y2= ys[:, :,:, H * W:]
            y1 = y1[:, 0] + y1[:, 2] + y1[:, 1].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, C, -1) + \
                y1[:, 3].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, C, -1)

            y2 = y2[:, 0] + y2[:, 2] + y2[:, 1].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, C,-1) + \
                 y2[:, 3].view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, C, -1)
    ...
    class CrossMerge_multimodal_k4(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor):
            B, K, D, H, W,_ = ys.shape
            ys =ys.view(B, K, -1, 2 * H * W)
            B, K, D, L = ys.shape
            # ctx.shape = (H, W)
            # ys = ys.view(B, K, D, -1)
            ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1])  # B, d, 2 * H * W, broadcast
            y=ys[:, 0] + ys[:, 1].view(B, -1, W, H,2).transpose(dim0=2, dim1=3).contiguous().view(B, D, -1)
            return y[:, :, 0:L // 2], y[:, :, L // 2:L]
    ...
    def cross_selective_scan_multimodal_k4(
            x_rgb: torch.Tensor = None,
            x_e: torch.Tensor = None,
            x_proj_weight: torch.Tensor = None,
            x_proj_bias: torch.Tensor = None,
        ...
        x_fuse = CrossScan_multimodal_local.apply(x_rgb, x_e)  # B, C, H, W -> B, 4, C, 2 * H * W
        ...
        y_rgb, y_e = ys[:,:,:,:H*W], ys[:,:,:,H*W:]
        y_rgb=y_rgb.view(B, K, -1,  H , W)
        y_e = y_e.view(B, K, -1,  H , W)
        # y_rgb, y_e = CrossMerge_multimodal.apply(ys)
        y_rgb = CrossMerge_local.apply(y_rgb)
        y_e = CrossMerge_local.apply(y_e)
        y_rgb = y_rgb.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_e = y_e.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_rgb = out_norm1(y_rgb).to(x_rgb.dtype)
        y_e = out_norm2(y_e).to(x_e.dtype)
...
class ConMB_SS2D_Region(nn.Module):
    '''
    ...
    def __init__(
        ...
        self.patch_embed = Stem(
           in_chans=self.d_inner, outer_dim=self.d_inner*4, inner_dim=self.d_inner)
    ...
    def forward_corev2_multimodal(self, x_rgb: torch.Tensor, x_e: torch.Tensor, nrows=-1):
        return cross_selective_scan_multimodal_k4(
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x_rgb = self.in_proj(x_rgb)  # (B,H,W,D)
...
class SSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        xz =self.in_proj(x)
        xz = xz.permute(0, 2, 1)
        x_dbl = self.x_proj(rearrange(xz, "b d l -> (b l) d"))  # (bl d)
...
class Cross_Mamba_Attention_SSM(nn.Module):
    def __init__(
        ...
        self.x_proj_3 = nn.Linear(2*self.d_state,  self.d_state, bias=False, **factory_kwargs)
        self.x_proj_4 = nn.Linear(2*self.d_state,  self.d_state , bias=False, **factory_kwargs)
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        C_fuse_rgb= self.x_proj_3(torch.concat([C_rgb,C_e],dim=1))
        C_fuse_e = self.x_proj_4(torch.concat([C_rgb, C_e], dim=1))
        # print(A_rgb.shape)
        C_rgb = rearrange(C_fuse_rgb, "(b l) dstate -> b dstate l", l=L).contiguous()
        C_e = rearrange(C_fuse_e, "(b l) dstate -> b dstate l", l=L).contiguous()
...
class ConcatMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x = self.drop_path(self.op(x_rgb, x_e))  # x_rgb + x_e +
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        return x
